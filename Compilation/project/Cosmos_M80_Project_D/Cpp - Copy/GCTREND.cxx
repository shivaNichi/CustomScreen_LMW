...
{
	GCTREND *pPanel = (GCTREND*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCTREND( GBaseObject *pObject )
{
	GCTREND *pPanel = new GCTREND;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCTREND;
...
GPropertyInfo* GCTREND::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCTREND::KEYACTIONOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	GBaseObject *pChild;
...
long GCTREND::KEYACTIONOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	GBaseObject *pChild;
	...
	switch(lLParam)
	{
		...
		case GK_F10:
					GESetEvent(GECreateEventMessage(GM_SHOWPANEL, GCSGetScreen(GetGBaseObject()), TOOL_LOAD_TYPE_SCREEN, 0), FALSE);
					break;
		default:
			break;
	}
...
long GCTREND::KEYACTIONOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
	GBaseObject *pChild;
	pChild = GCSGetChild(GetGBaseObject(), KEYACTION);
...
long GCTREND::KEY_BACKOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	GESetEvent(GECreateEventMessage(GM_SHOWPANEL, GCSGetScreen(GetGBaseObject()), TOOL_LOAD_TYPE_SCREEN, 0), FALSE);
	return TRUE;
}
