...
long GCMaglist::KEYACTIONOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	...
	switch(lLParam)
	{
		...
		case GK_F5:
					...
					//for Checking R69 data		
					melGetData(0, (ADR_MACHINE(1)|ADR_BASE_SYSTEM|ADR_SYSTEM(1)|ADR_GROUND(0)), M_SEC_PLC_DEV_WORD, 
																					 M_SSEC_PLWRD_R_1SHOT(69),1, &sIconR69, T_SHORT); // To get the Data Protection Status
				
					// Checking for JOG mode
					if(lRunProg == 1 && permitData ==1 && sIconR69 != 65535)	
					{
							sConfirmWindowNo = 1;
							sHelpFlg = 2;
					...
					else
					{
						...
						else if(permitData!=1)
							setWarningMessage(EDIT_PERMIT_ERR); 
						else if(sIconR69 == 65535)
							setWarningMessage(CAUTION_ERR); 
					}
					break;
		case GK_F6:
				// Pot Lock Button
...
long GCMaglist::User(long lLParam, long lUParam)
{
	...
	switch(lLParam)
	{
		...
		case 4:	if(pData != NULL){
				...
				break;
		case 5:	if(pData != NULL){
					sConfirmWindowNo = 0;
					sHelpFlg =0;
				}
				break;
		case 6:	if(pData != NULL){
					MagClearData();					// Clear ALL Tool Data
					setWarningMessage(MAG_CLEAR_COMPLETE); 
					sConfirmWindowNo = 0;
					sHelpFlg =0;	
...
void GCMaglist::setWarningMessage(int warnnum)
{
	...
	if(warnnum == MAKE_LOWER_TOOL_ZERO_ERR)
		...
		GRCMultiByteToUnicode(GRCLoadNowLanguageStr(), szText, "Can't be clear, Please Press Emergency Stop !", sizeof(szText)/sizeof(GTCHAR));
...
/************************************************************************************/
void GCMaglist::GetPotInfo(PotInfo *pt)
{
...
/************************************************************************************/
void GCMaglist::SetPotInfo(PotInfo *pt)
{
