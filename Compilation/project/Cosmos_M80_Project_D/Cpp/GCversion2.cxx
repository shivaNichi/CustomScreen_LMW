...
{
	GCversion2 *pPanel = (GCversion2*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCversion2( GBaseObject *pObject )
{
	GCversion2 *pPanel = new GCversion2;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCversion2;
...
#include "GCversion2.prc"

short GCversion2::CreateChildren( )
{
	GBaseObject *pChild;
...
GPropertyInfo* GCversion2::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCversion2::KEYACTIONOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	GBaseObject *pChild;
...
long GCversion2::KEYACTIONOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	GBaseObject *pChild;
	GTCHAR inpBuff[10];
	char sData[10];
...
long GCversion2::KEYACTIONOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
	GBaseObject *pChild;
	pChild = GCSGetChild(GetGBaseObject(), KEYACTION);
...
long GCversion2::KEY_BACKOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	GESetEvent(GECreateEventMessage(GM_SHOWPANEL, GCSGetScreen(GetGBaseObject()), DIAGNOSIS_SCREEN, 0), FALSE);
	return TRUE;
}
...
/****************************************************************************************************/
void GCversion2::ReadIconStatus()
{
	//ICON 1
