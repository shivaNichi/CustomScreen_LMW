...
{
	GCLubrication_Sys *pPanel = (GCLubrication_Sys*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCLubrication_Sys( GBaseObject *pObject )
{
	GCLubrication_Sys *pPanel = new GCLubrication_Sys;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCLubrication_Sys;
 ...
 *	<description>	:	コンストラクタ
 */
GCLubrication_Sys::GCLubrication_Sys()
{
}
...
#include "GCLubrication_Sys.prc"

short GCLubrication_Sys::CreateChildren( )
{
	GBaseObject *pChild;
...
GPropertyInfo* GCLubrication_Sys::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCLubrication_Sys::DATAOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	Fetchdata();
...
long GCLubrication_Sys::DATAOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	switch(lLParam)
	{
		case GK_F1:
...
long GCLubrication_Sys::DATAOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
		GBaseObject *pChild;
...
void GCLubrication_Sys::Fetchdata()
{
	...
	char *s;
		...
		char sDestFile[] = "M01:\\PRG\\MMACRO\\LUBRICATION.CSV";
...
void GCLubrication_Sys::displaydata()
{
...
bool GCLubrication_Sys::Readstatus(char *cInputData)
{
	char prgststatus[20];
	STRINGTYPE str1;
	str1.lBuffSize=20;
...
void GCLubrication_Sys::setWarningMessage(int warnnum)
{
	GBaseObject *pLbl;
	GTCHAR szText[60];
	...
	if(warnnum == 1)
		GRCMultiByteToUnicode(GRCLoadNowLanguageStr(), szText, "LUBRICATION.CSV file not Present", sizeof(szText)/sizeof(GTCHAR));
	GCSTextboxSetString( pLbl, szText );
}

long GCLubrication_Sys::BTNEOSOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
		ScreenTransition(EOS_SCREEN);
	return TRUE;
}

long GCLubrication_Sys::BTNEXITOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( HMI_SCREEN);
	return TRUE;
}

long GCLubrication_Sys::BTNMCOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( SUBSYS_SCREEN);
	return TRUE;
}
