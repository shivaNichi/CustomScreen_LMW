...
{
	GCATC_Info *pPanel = (GCATC_Info*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCATC_Info( GBaseObject *pObject )
{
	GCATC_Info *pPanel = new GCATC_Info;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCATC_Info;
...
#include "GCATC_Info.prc"

short GCATC_Info::CreateChildren( )
{
	GBaseObject *pChild;
...
GPropertyInfo* GCATC_Info::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCATC_Info::DATAOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	Fetchdata();
...
long GCATC_Info::DATAOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	switch(lLParam)
	{
		case GK_F1:
...
long GCATC_Info::DATAOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
		GBaseObject *pChild;
...
void GCATC_Info::Fetchdata()
{
	...
	char *s;
		...
		char sDestFile[] = "M01:\\PRG\\MMACRO\\ATC.CSV";
...
bool GCATC_Info::Readstatus(char *cInputData)
{
	char prgststatus[20];
	STRINGTYPE str1;
	str1.lBuffSize=20;
...
void GCATC_Info::setWarningMessage(int warnnum)
{
	GBaseObject *pLbl;
	GTCHAR szText[60];
	...
	if(warnnum == 1)
		GRCMultiByteToUnicode(GRCLoadNowLanguageStr(), szText, "ATC.CSV file not Present", sizeof(szText)/sizeof(GTCHAR));
