...
{
	GCChip_CONVY_Sys *pPanel = (GCChip_CONVY_Sys*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCChip_CONVY_Sys( GBaseObject *pObject )
{
	GCChip_CONVY_Sys *pPanel = new GCChip_CONVY_Sys;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCChip_CONVY_Sys;
 ...
 *	<description>	:	コンストラクタ
 */
GCChip_CONVY_Sys::GCChip_CONVY_Sys()
{
}
...
#include "GCChip_CONVY_Sys.prc"

short GCChip_CONVY_Sys::CreateChildren( )
{
	GBaseObject *pChild;
...
GPropertyInfo* GCChip_CONVY_Sys::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCChip_CONVY_Sys::DATAOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	Fetchdata();
...
long GCChip_CONVY_Sys::DATAOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	switch(lLParam)
	{
		case GK_F1:
...
long GCChip_CONVY_Sys::DATAOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
		GBaseObject *pChild;
...
void GCChip_CONVY_Sys::Fetchdata()
{
	...
	char *s;
		...
		char sDestFile[] = "M01:\\PRG\\MMACRO\\CHIPCONVEY.CSV";
...
void GCChip_CONVY_Sys::displaydata()
{
	...
	for(int i = 0; i < NO_BTN; i++)
	{
		...
		if(i==5)
		bOutbit2=bit;
		bOutbit1=bOutbit1|bOutbit2;
		GCSButtonSetStatus(pBtn[7], bOutbit1);
	
	}
}
bool GCChip_CONVY_Sys::Readstatus(char *cInputData)
{
	char prgststatus[20];
	STRINGTYPE str1;
	str1.lBuffSize=20;
...
void GCChip_CONVY_Sys::setWarningMessage(int warnnum)
{
	GBaseObject *pLbl;
	GTCHAR szText[60];
	...
	if(warnnum == 1)
		GRCMultiByteToUnicode(GRCLoadNowLanguageStr(), szText, "CHIPCONVEY.CSV file not Present", sizeof(szText)/sizeof(GTCHAR));
	GCSTextboxSetString( pLbl, szText );
}

long GCChip_CONVY_Sys::BTNEOSOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition(EOS_SCREEN);
	return TRUE;
}

long GCChip_CONVY_Sys::BTNEXITOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( HMI_SCREEN);
	return TRUE;
}

long GCChip_CONVY_Sys::BTNMCOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( SUBSYS_SCREEN);
	return TRUE;
}
