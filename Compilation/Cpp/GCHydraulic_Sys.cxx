...
{
	GCHydraulic_Sys *pPanel = (GCHydraulic_Sys*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCHydraulic_Sys( GBaseObject *pObject )
{
	GCHydraulic_Sys *pPanel = new GCHydraulic_Sys;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCHydraulic_Sys;
 ...
 *	<description>	:	コンストラクタ
 */
GCHydraulic_Sys::GCHydraulic_Sys()
{
}
...
#include "GCHydraulic_Sys.prc"

short GCHydraulic_Sys::CreateChildren( )
{
	GBaseObject *pChild;
...
GPropertyInfo* GCHydraulic_Sys::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCHydraulic_Sys::DATAOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	Fetchdata();
...
long GCHydraulic_Sys::DATAOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	switch(lLParam)
	{
		case GK_F1:
...
long GCHydraulic_Sys::DATAOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
		GBaseObject *pChild;
...
void GCHydraulic_Sys::Fetchdata()
{
	...
	char *s;
		...
		char sDestFile[] = "M01:\\PRG\\MMACRO\\HYDRAULIC.CSV";
...
void GCHydraulic_Sys::displaydata()
{
...
bool GCHydraulic_Sys::Readstatus(char *cInputData)
{
	char prgststatus[20];
	STRINGTYPE str1;
	str1.lBuffSize=20;
...
void GCHydraulic_Sys::setWarningMessage(int warnnum)
{
	GBaseObject *pLbl;
	GTCHAR szText[60];
	...
	if(warnnum == 1)
		GRCMultiByteToUnicode(GRCLoadNowLanguageStr(), szText, "HYDRAULIC.CSV file not Present", sizeof(szText)/sizeof(GTCHAR));
	GCSTextboxSetString( pLbl, szText );
}

long GCHydraulic_Sys::BTNEOSOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition(EOS_SCREEN);
	return TRUE;
}

long GCHydraulic_Sys::BTNEXITOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( HMI_SCREEN);
	return TRUE;
}

long GCHydraulic_Sys::BTNMCOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( SUBSYS_SCREEN);
	return TRUE;
}
