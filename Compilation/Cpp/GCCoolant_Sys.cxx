...
{
	GCCoolant_Sys *pPanel = (GCCoolant_Sys*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCCoolant_Sys( GBaseObject *pObject )
{
	GCCoolant_Sys *pPanel = new GCCoolant_Sys;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCCoolant_Sys;
...
#include "GCCoolant_Sys.prc"

short GCCoolant_Sys::CreateChildren( )
{
	GBaseObject *pChild;
...
GPropertyInfo* GCCoolant_Sys::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCCoolant_Sys::DATAOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	Fetchdata();
...
long GCCoolant_Sys::DATAOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	switch(lLParam)
	{
		case GK_F1:
...
long GCCoolant_Sys::DATAOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
		GBaseObject *pChild;
...
void GCCoolant_Sys::Fetchdata()
{
	...
	char *s;
		...
		char sDestFile[] = "M01:\\PRG\\MMACRO\\COOLANT.CSV";
...
void GCCoolant_Sys::displaydata()
{
...
bool GCCoolant_Sys::Readstatus(char *cInputData)
{
	char prgststatus[20];
	STRINGTYPE str1;
	str1.lBuffSize=20;
...
void GCCoolant_Sys::setWarningMessage(int warnnum)
{
	GBaseObject *pLbl;
	GTCHAR szText[60];
	...
	if(warnnum == 1)
		GRCMultiByteToUnicode(GRCLoadNowLanguageStr(), szText, "COOLANT.CSV file not Present", sizeof(szText)/sizeof(GTCHAR));
	GCSTextboxSetString( pLbl, szText );
}

long GCCoolant_Sys::BTNEOSOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition(EOS_SCREEN);
	return TRUE;
}

long GCCoolant_Sys::BTNEXITOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( HMI_SCREEN);
	return TRUE;
}

long GCCoolant_Sys::BTNMCOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( SUBSYS_SCREEN);
	return TRUE;
}
