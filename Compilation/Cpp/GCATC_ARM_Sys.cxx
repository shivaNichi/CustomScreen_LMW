...
{
	GCATC_ARM_Sys *pPanel = (GCATC_ARM_Sys*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCATC_ARM_Sys( GBaseObject *pObject )
{
	GCATC_ARM_Sys *pPanel = new GCATC_ARM_Sys;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCATC_ARM_Sys;
...
#include "GCATC_ARM_Sys.prc"

short GCATC_ARM_Sys::CreateChildren( )
{
	GBaseObject *pChild;
...
GPropertyInfo* GCATC_ARM_Sys::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCATC_ARM_Sys::DATAOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	Fetchdata();
...
long GCATC_ARM_Sys::DATAOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	switch(lLParam)
	{
		case GK_F1:
				...
				/*else
					...
					ScreenTransition( ATC_MAG_SCREEN);
...
long GCATC_ARM_Sys::DATAOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
		GBaseObject *pChild;
...
void GCATC_ARM_Sys::Fetchdata()
{
	...
	char *s;
		...
		char sDestFile[] = "M01:\\PRG\\MMACRO\\ARM.CSV";
...
void GCATC_ARM_Sys::displaydata()
{
...
bool GCATC_ARM_Sys::Readstatus(char *cInputData)
{
	char prgststatus[20];
	STRINGTYPE str1;
	str1.lBuffSize=20;
...
void GCATC_ARM_Sys::setWarningMessage(int warnnum)
{
	GBaseObject *pLbl;
	GTCHAR szText[60];
	...
	if(warnnum == 1)
		GRCMultiByteToUnicode(GRCLoadNowLanguageStr(), szText, "ARM.CSV file not Present", sizeof(szText)/sizeof(GTCHAR));
	GCSTextboxSetString( pLbl, szText );
}


long GCATC_ARM_Sys::BTNEOSOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition(EOS_SCREEN);
	return TRUE;
}

long GCATC_ARM_Sys::BTNEXITOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( HMI_SCREEN);
	return TRUE;
}

long GCATC_ARM_Sys::BTNMAGOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( ATC_MAG_SCREEN);
	return TRUE;
}
