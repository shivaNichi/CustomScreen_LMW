...
{
	GCGCODE *pPanel = (GCGCODE*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCGCODE( GBaseObject *pObject )
{
	GCGCODE *pPanel = new GCGCODE;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCGCODE;
...
GPropertyInfo* GCGCODE::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCGCODE::DATAOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
	...
	switch(lLParam)
	{
		...
		case GK_F1:
		   ...
		   M_SEC_PLC_DEV_BIT,40001,0, (void *)&ldata, T_LONG); */
...
long GCGCODE::DATAOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
...
long GCGCODE::HIST_TABLEOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	return TRUE;
}

//void GCGCODE::UpdateStatus()
//{
//	GBaseObject *pTbl;
...
//	{
//		bool bit=Readstatus(SMcodeDet[j].Adress);
//			if(bit==1)
//			{	
...
//	if(AlarmOnbit[i]>=0)
//	{
//		if(strlen(SMcodeDet[AlarmOnbit[i]+ulIndex].Adress)>1)
//		{
//
//			sprintf(KeyData, "%s",SMcodeDet[AlarmOnbit[i]+ulIndex].Adress  );
//			GRCMultiByteToUnicode(0, KeyText, KeyData,  sizeof(KeyText)/sizeof(GTCHAR));
//			GCSNCTableSetCellString(pTbl, KeyText, i, 0, &sretErr);
//	
//
//			sprintf(KeyData, "%s",SMcodeDet[AlarmOnbit[i]+ulIndex].Comment );
//			GRCMultiByteToUnicode(0, KeyText, KeyData, sizeof(KeyText)/sizeof(GTCHAR));
...
//	}
//}
bool GCGCODE::Readstatus(char *cInputData)
{
	char prgststatus[20];
	STRINGTYPE str1;
	str1.lBuffSize=20;
...
long GCGCODE::DATAOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
...
void GCGCODE::Fetchdata(long sOffset)
{
	...
	long countsize=0;
		...
		char sDestFile[] = "M01:\\PRG\\MMACRO\\GCODE.CSV";
		...
		while(countsize!=(long)stat.lFileSize&&countsize<=((long)stat.lFileSize-1))
		{
			...
			int rem =ulLineCcount%15;
				if(rem==0)
					rem=(ulLineCcount-15);
				else
				rem=ulLineCcount-rem;


				for(int j=rem,k=0;j<ulLineCcount;j++,k++)
				{
				s1=strtok(sAdress[k],",");
				//s1=strtok(NULL,",");
				if(s1!=NULL)
				{
				strcpy(SMcodeDet[j].Adress,s1);
				s1=strtok(NULL,",");
				if(s1!=NULL)
				strcpy(SMcodeDet[j].Comment,s1);
				}
...
melFSCloseFile(0, (ADR_MACHINE(1)|ADR_BASE_SYSTEM|ADR_SYSTEM(1)|ADR_GROUND(0)), lFileDesc);
		...
		/*	int i =0;
			...
			while(s!=NULL)
			{
				if(strncmp( s,"A",1)==0)
				{
				strcpy(SMcodeDet[i].Adress,s);
				s=strtok(NULL,",");
				if(s!=NULL)
				strcpy(SMcodeDet[i].Comment,s);
				s=strtok(NULL,"\n");
				if(s!=NULL)
				strcpy(SMcodeDet[i].Remedy,s);
					i++;
				}
				s=strtok(NULL,",");*/
...
void GCGCODE::ScreenTransition(short ScreenNo)
{
...
void GCGCODE::displaydata()
{
	...
	for(i = 0; i < lEndCnt; i++)
	{
		...
		sprintf(KeyData, "%d",i+ulIndex+1 );
		GRCMultiByteToUnicode(0, KeyText, KeyData,  sizeof(KeyText)/sizeof(GTCHAR));
		GCSNCTableSetCellString(pTbl, KeyText, i, 0, &ErrCode);

		sprintf(KeyData, "%s",SMcodeDet[i+ulIndex].Adress  );
		GRCMultiByteToUnicode(0, KeyText, KeyData,  sizeof(KeyText)/sizeof(GTCHAR));
		GCSNCTableSetCellString(pTbl, KeyText, i, 1, &ErrCode);
	

		sprintf(KeyData, "%s",SMcodeDet[i+ulIndex].Comment );
		GRCMultiByteToUnicode(0, KeyText, KeyData, sizeof(KeyText)/sizeof(GTCHAR));
...
//char* GCGCODE::Fetchcomment(char *cAdrss)
//{
//
//	DWORD dwStatus;
//	FS_STAT stat;
...
//
//}
//void GCGCODE::LoadHtmlFile(char *cAdrss)
//{
...
long GCGCODE::HOMEBTNOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
...
long GCGCODE::BTNMCODEOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition( MCODE_SCREEN);
	return TRUE;
}
