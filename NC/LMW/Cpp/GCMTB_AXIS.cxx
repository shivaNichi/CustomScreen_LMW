...
{
	GCMTB_AXIS *pPanel = (GCMTB_AXIS*)pObject->pSelf;
	delete pPanel;
	pObject->pSelf = NULL;
}	
short GCINewGCMTB_AXIS( GBaseObject *pObject )
{
	GCMTB_AXIS *pPanel = new GCMTB_AXIS;
	if( pPanel == NULL ){
		return FALSE;
	}
	...
	pObject->GCIDeleteObject = &GCIDeleteGCMTB_AXIS;
...
#include "GCMTB_AXIS.prc"

short GCMTB_AXIS::CreateChildren( )
{
	GBaseObject *pChild;
...
GPropertyInfo* GCMTB_AXIS::GetPropertyInfo(const GBaseWindowProperty* pImport, unsigned short* pGPropertyInfoCnt)
{
...
long GCMTB_AXIS::DATAOnCreate(unsigned short usMessage, long lLParam, long lUParam)
{
	Cursor=0;
	SetColor(Cursor);
	ucChkStatus=0;
...
long GCMTB_AXIS::DATAOnKeyPress(unsigned short usMessage, long lLParam, long lUParam)
{
...
long GCMTB_AXIS::DATAOnTimer(unsigned short usMessage, long lLParam, long lUParam)
{
	GBaseObject *pChild;
...
void GCMTB_AXIS::SetColor(short cur_pos)
{
	...
	short sCursorNum = MAX_TXT;
		...
		GCSNCDataTextboxSetNormalColor(pTxtB[4] , DEFAULT_COLOR);		
		GCSNCDataTextboxSetNormalColor(pTxtB[5] , DEFAULT_COLOR);		
...
void GCMTB_AXIS::SetData(short cur_pos)
{
...
void GCMTB_AXIS::ScreenTransition(short ScreenNo)
{
...
long GCMTB_AXIS::BTNHOMEOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition(MTB_PASSWORD);
	return TRUE;
}
long GCMTB_AXIS::GetEditKeyProtection()
{
...
long GCMTB_AXIS::BTNENABLEOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
		GBaseObject *pChkBox;
	pChkBox = GCSGetChild(GetGBaseObject(), CHKBOX1);
	ucChkStatus=!ucChkStatus;
...
void GCMTB_AXIS:: ClearWarningMessage()
{
	GBaseObject *pLbl;
	pLbl = GCSGetChild(GetGBaseObject(), MSGBOX);
...
void  GCMTB_AXIS::SetWarningMessage(int warnnum)
{
	GBaseObject *pLbl;
	GTCHAR szText[WARNTEXT_SIZE];
...
void GCMTB_AXIS:: SetRange(int ScreenNo,short cur_pos)
...
long GCMTB_AXIS::checkRange(char *dat,int ScreenNo, int cur_pos)
{
//	GBaseObject *pErr;
...
long GCMTB_AXIS::BTNSPORIENTOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition(MTB_SPORIENT);
	return TRUE;
}

long GCMTB_AXIS::BTNSPSPEEDOnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	ScreenTransition(MTB_SPSPEED);
	return TRUE;
}
void GCMTB_AXIS::StoreData()
{
	...
	short lErrFlag;
		...
		pTxtB[5] = GCSGetChild(pPanel, TXT6);
			...
			melGetData(0, (ADR_MACHINE(1)|ADR_BASE_SYSTEM|ADR_SYSTEM(1)|ADR_GROUND(0)), M_SEC_PLC_DEV_WORD,M_SSEC_PLWRD_R_1SHOT_EXT(28627+i) ,0, (void *)&Cmpdata, T_LONG);
			if(TempData!=Cmpdata)
			melSetData(0, (ADR_MACHINE(1)|ADR_BASE_SYSTEM|ADR_SYSTEM(1)|ADR_GROUND(0)), M_SEC_PLC_DEV_WORD,M_SSEC_PLWRD_R_1SHOT_EXT(28627+i),0, &TempData, T_LONG);
			}
...
long GCMTB_AXIS::CHKBOX1OnClick(unsigned short usMessage, long lLParam, long lUParam)
{
	GBaseObject *pChkBox;
	pChkBox = GCSGetChild(GetGBaseObject(), CHKBOX1);
	ucChkStatus=!ucChkStatus;
